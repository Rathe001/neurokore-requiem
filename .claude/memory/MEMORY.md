...
- [Host migration plan](project_host_migration_plan.md) — Multi-session work. Session 1 scaffolding shipped behind DebugConfig.host_migration_enabled (off by default). Election + rebind + re-authority code path; needs cross-account playtest to verify. Roadmap in docs/host-migration.md.
...
- [Behavior mods wiring patterns](project_behavior_mods_wiring.md) — 9 of 24 mods implemented (2026-05-24); passive-query vs event-hook patterns; offensive buffs aggregate via behavior_mod_damage_mult()
...
- [Codebase audit (2026-06-03)](project_audit_2026_06_03.md) — SHIPPED. All 13 findings closed across 6 phases (1c→1b→1a→2a→2b→3). ~1100 LOC net removed. Commit range 1c7cc4c..9339c96.
- [MP damage/heal routing](project_mp_damage_heal_routing.md) — PrototypePlayer.apply_damage + apply_heal static helpers (+ paired RPCs) route across MP authority. Every enemy → player damage source must go through these; direct take_damage early-returns on non-authority.
...
- [LiquidLayer stamp lifecycle](project_liquid_layer_stamp_lifecycle.md) — one-frame stamps must outlive the SubViewport render: use create_timer(0.1), not process_frame.connect, or stamps silently never deposit (bit me on footprints)
