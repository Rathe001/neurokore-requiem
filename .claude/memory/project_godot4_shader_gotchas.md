...
**10. `VERTEX` in `fragment()` is in VIEW space, not model-local.** It's model-local only inside `vertex()`. If you need mesh-local coordinates in the fragment stage (radial vignettes, object-space distance falloffs, anything keyed off the mesh's own coordinate frame), capture VERTEX in `vertex()` and route it through a `varying`. Reading VERTEX in fragment without doing this gives camera-relative coords — silently wrong, no error. Bit me on the hammer-crater alpha vignette: the smoothstep mask never reached the edge because it was reading view-space distance instead of mesh distance, so the rectangular mesh silhouette stayed hard.
