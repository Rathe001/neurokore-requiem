...
> **Status: In progress.** Phases 0 through 8 + 2C are shipped and on `main`. All planned networking phases are complete; v0.2.0 added cross-client projectile + channel-beam visibility, in-game chat, and a graceful host-disconnect overlay. See [Implementation phases](#implementation-phases) below for the full sequence and what each one covers.
>
...
### Phase 8 — Coop visibility + UX ✅ (v0.2.0)

Once enemies + combat + loot + drop-in were live, the remaining MP gap was *seeing what your teammates were doing*. Phase 8 closes that:

1. **Player projectile replication** — every player-fired projectile (laser, plasma, SMG/LMG/sniper bullets, RPG rockets, shotgun pellets, airstrike rockets) broadcasts a `_rpc_projectile` (or `_rpc_shotgun_burst` for batched 9/18-pellet shotgun casts) via `CombatVisuals`. Remote peers spawn a *ghost projectile* (`PrototypeProjectile.is_ghost = true`) that runs the same mesh, trail, smoke, and look_at math as the host's projectile, but its collision mask is forced to world-only and damage fields are zeroed — visuals + travel only. Impact bursts and explosions arrive via their own one-shot replicated calls, so ghost projectiles release silently on wall contact to avoid double visuals. Shotgun pellet RPCs are batched into a single `PackedVector3Array` so a 9-pellet cast is one RPC, not nine.

2. **Channel-beam visual replication** — Energy Accelerator's flame is a *persistent* visual (not a one-shot like the lightning arcs the Taser already replicates), so it needs start/stop events plus per-frame orientation. The firing peer broadcasts `_rpc_channel_flame_start(damage_type, weapon_range)` on the player node itself (authority-routed RPC) on `_start_channel`, and `_rpc_channel_flame_stop` on `_stop_channel`. Remote peers attach a flame visual to that player's avatar; orientation tracks the remote player's facing direction (`visual.basis.-z`) since the cursor isn't replicated.

3. **In-game chat** — new `GameplayChatState` autoload mirrors the existing `GlobalChatState` pattern but scopes to `NetState.lobby_id`. Sends via `Steam.sendLobbyChatMsg`, receives via `Steam.lobby_message` (filtered by lobby_id), exposes a `chat_received(steam_id, name, text)` signal and a `typing: bool` flag the player input handler polls to gate movement / skill / hotkey input while the chat input is focused. UI panel `GameplayChatPanel` at the bottom-left of the HUD shows the last 6 lines, fades 8s after arrival, displays sender names in their class color. Each peer publishes their `class_id` (spec_id when specced, else origin) under the `class` member-data key on lobby join; remote class color is read from `Steam.getLobbyMemberData`.

4. **Host-disconnect handling** — when peer 1 disconnects mid-session, clients used to enter a frozen-but-running limbo (no enemy spawns, no state sync, no host snapshots). NetState now subscribes to `multiplayer.peer_disconnected` only on the client side (after `_create_client_peer`) and emits a `host_disconnected` signal when peer_id == 1. `PrototypeRoot` listens and spawns a `HostDisconnectedScreen` overlay — full-screen muted backdrop, "HOST DISCONNECTED" title, single "Return to Main Menu" button. NetState's `_teardown_peer` disconnects the handler before closing the peer so a deliberate `leave_lobby` doesn't trigger the overlay on its own client.

Performance hot paths addressed in the same phase: visual-replication anchors (the temporary Node3D `CombatVisuals` builds at each RPC's origin point) are pooled instead of allocated per RPC; lightning arc visuals reuse a cached unit PlaneMesh + templated ShaderMaterial; the airstrike marker mesh + material are cached; the Energy Accelerator's wall-clip raycast uses a member-cached `PhysicsRayQueryParameters3D` instead of allocating one per physics frame.

Host migration remains out of scope. The architecture is a fixed peer-1 server model with no failover authority — when peer 1 drops, clients get the disconnect overlay and exit cleanly. Real migration would require Steam `setLobbyOwner` + reauthoring every per-entity `set_multiplayer_authority` + replaying host state into the new authority, which is a multi-week project on its own.
