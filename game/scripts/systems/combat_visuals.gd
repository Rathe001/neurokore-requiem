...
func spawn_hammer_impact(host: Node3D, radius: float = PrototypeAttackIndicator.HAMMER_IMPACT_RADIUS) -> void:
	PrototypeAttackIndicator.spawn_hammer_impact(host, radius)
	if NetState.is_in_lobby():
		var cv: Node = host.get_node(_AUTOLOAD_PATH)
		cv._rpc_hammer_impact.rpc(host.global_position, host.is_in_group(&"player"), radius)

@rpc("any_peer", "call_remote", "unreliable")
func _rpc_hammer_impact(origin: Vector3, is_player: bool, radius: float = PrototypeAttackIndicator.HAMMER_IMPACT_RADIUS) -> void:
	var anchor := _acquire_anchor(origin, is_player)
	PrototypeAttackIndicator.spawn_hammer_impact(anchor, radius)
	_release_anchor(anchor)
